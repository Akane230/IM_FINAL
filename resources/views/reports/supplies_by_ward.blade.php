<x-app-layout>
    ...
    <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8 text-black space-y-6">
        {{-- Ward Selection --}}
        <form method="GET" action="{{ route('reports.suppliesByWard') }}" class="bg-white p-4 rounded shadow border">
            <label for="wardID" class="font-bold mr-2">Select Ward:</label>
            <select name="wardID" id="wardID" onchange="this.form.submit()" class="border p-2 rounded">
                @foreach ($wards as $w)
